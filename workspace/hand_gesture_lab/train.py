...
class GestureLSTM(nn.Module):
    def __init__(self, input_dim, num_classes):
        ...
        # Updated model capacity (96->112, 48->64) and reduced dropout (0.4->0.3)
        self.lstm1 = nn.LSTM(input_dim, 112, batch_first=True, bidirectional=True)
        self.ln1 = nn.LayerNorm(224)  # 2 * 112
        self.dropout1 = nn.Dropout(0.3)
        
        self.lstm2 = nn.LSTM(224, 64, batch_first=True, bidirectional=True)
        self.ln2 = nn.LayerNorm(128)  # 2 * 64
        self.dropout2 = nn.Dropout(0.3)
        
        # Classification head (applied per frame)
        # Attention projection to a scalar score per time step
        self.attn_proj = nn.Linear(128, 1)
        # Classification head operates on the attention‑weighted context vector
    ...
    def forward(self, x):
        ...
        # ---- ATTENTION ----
        # Compute attention scores (B, T, 1) then softmax over T
        attn_scores = self.attn_proj(out)                # (B, T, 1)
        attn_weights = torch.softmax(attn_scores.squeeze(-1), dim=1).unsqueeze(-1)  # (B, T, 1)
        # Weighted sum to obtain context vector (B, 128)
        context = torch.sum(attn_weights * out, dim=1)
        
        # Classification on the context vector
        logits = self.classifier(context)   # (B, num_classes)
        
        # Keep the existing temporal smoothing logic for compatibility –
        # we treat the context logits as a constant across time steps.
        # Create a dummy sequence of length T filled with the same logits
        logits_seq = logits.unsqueeze(1).repeat(1, T, 1)  # (B, T, num_classes)
        smoothed = torch.zeros_like(logits_seq)
        smoothed[:, 0, :] = logits_seq[:, 0, :]
        alpha = 0.6
        for t in range(1, T):
            smoothed[:, t, :] = alpha * logits_seq[:, t, :] + (1 - alpha) * smoothed[:, t-1, :]
        # Return the final smoothed logits (B, num_classes)
        return smoothed[:, -1, :]
